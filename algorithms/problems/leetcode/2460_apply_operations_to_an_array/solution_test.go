...
in the ith operation (0-indexed), you will apply the following on
the ith element of nums:
- If nums[i] == nums[i + 1], then multiply nums[i] by 2 and set
  nums[i + 1] to 0. Otherwise, you skip this operation.

After performing all the operations, shift all the 0's to the end of
the array.
...
func TestApplyArrayOperations(t *testing.T) {
	...
	ApplyArrayOperations(nums)
}

func TestApplyArrayOperations2(t *testing.T) {
	nums := []int{0, 1}
	ApplyArrayOperations(nums)
}

func ApplyArrayOperations(nums []int) []int {
	n := len(nums)
	...
	zeroes := n - len(nums)
	nums = append(nums, make([]int, zeroes)...)
