...
It must be DFS week. The trick here is order. DFS will generate happy strings using a stack
but this will be in the reverse order. So we can use a BFS instead.

Happy strings are strings that do not have consecutive repeating characters.
a, ab, ac, aba, abc, acb, aca, acb, abc, aba, abac. We want the first lexico happy string.

Another idea. We can generate all happy strings, but this is likely the brute case.
*/

func TestKthHappyString(t *testing.T) {
